...
async function verifyPriceDrop() {
  ...
  const products: any = await executeQuery(
    'SELECT id, price_cache, msrp_price FROM products LIMIT 1'
  );
  ...
  await executeQuery(
    'DELETE wi FROM wishlist_items wi JOIN wishlists w ON wi.wishlist_id = w.id WHERE w.user_id = ? AND wi.product_id = ?',
  ...
  let wishlistId;
  const wls: any = await executeQuery(
    'SELECT id FROM wishlists WHERE user_id = ? AND is_default = 1',
    [testUserId]
  );
  if (!wls || wls.length === 0) {
    const res: any = await executeQuery(
      'INSERT INTO wishlists (user_id, name, is_default) VALUES (?, ?, ?)',
      [testUserId, 'My Wishlist', 1]
    );
  ...
  const originalRetailPrice = products[0].msrp_price;
  const originalBasePrice = products[0].price_cache;

  await executeQuery(
    'UPDATE products SET msrp_price = 1000000, price_cache = 800000 WHERE id = ?',
    [testProductId]
  );
  ...
  // Dump check
  const raw: any = await executeQuery('SELECT * FROM wishlist_items WHERE product_id = ?', [
    testProductId,
  ]);
  ...
  await executeQuery('UPDATE products SET msrp_price = ?, price_cache = ? WHERE id = ?', [
    originalRetailPrice,
    originalBasePrice,
    testProductId,
  ]);
  await executeQuery('DELETE FROM wishlist_items WHERE wishlist_id = ? AND product_id = ?', [
    wishlistId,
    testProductId,
  ]);
