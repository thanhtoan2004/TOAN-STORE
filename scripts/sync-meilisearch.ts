...
async function syncProducts() {
  ...
  try {
    ...
    const documents: ProductDocument[] = products.map((p) => {
      // In this project: price_cache is the actual selling price, msrp_price is MSRP (original)
      const price = Number(p.price_cache || p.msrp_price);
      return {
        id: p.id,
        name: p.name,
        slug: p.slug,
        ...
        price_cache: Number(p.price_cache),
        msrp_price: Number(p.msrp_price || 0),
        current_price: price,
    ...
    await index.updateSettings({
      ...
      sortableAttributes: ['current_price', 'created_at'],
      rankingRules: [
        'words',
        'typo',
        'proximity',
