...
Tài liệu API đầy đủ cho Nike Clone E-commerce. Tất cả API sử dụng prefix `/api/`.

> **Base URL**: `http://localhost:3000`  
> **Content-Type**: `application/json`  
> **Authentication**: JWT via Cookie (`nike_auth_session` / `nike_admin_session`)
...
| Rate Limit | Redis-backed, varies per endpoint group |
...
**Response:** `200 OK` — Set cookies: `nike_auth_session`, `nike_refresh_token`
...
**Response:** `200 OK` — Set cookie: `nike_admin_session`
...
Đăng xuất, xóa cookies và revoke refresh token từ Redis.
...
| GET | `/api/products/search` | Tìm kiếm (`?q=nike&limit=10`) |
...
| POST | `/api/cart/check-balance` | Kiểm tra tồn kho |
...
- `payment_received` → Stock finalize
...
> **Idempotency**: Cả VNPay và MoMo IPN đều kiểm tra transaction đã xử lý chưa trước khi update. Sử dụng `updateOrderStatus` để đi qua State Machine.
...
## 11. 🎟️ Coupons & Vouchers

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/coupons` | Danh sách coupon |
| POST | `/api/coupons/validate` | Kiểm tra mã (`{ "code": "NIKE2024" }`) |
| POST | `/api/coupons/apply` | Áp dụng mã |
| GET | `/api/voucher/list` | Danh sách voucher |
| POST | `/api/voucher/claim` | Nhận voucher |

---

## 12. ⚡ Flash Sales (`/api/flash-sales`)
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/cron` | Chạy cleanup (expired reservations, pending orders) |
| POST | `/api/cron/cleanup-orders` | Cleanup đơn hàng hết hạn |
| POST | `/api/cron/cleanup-reservations` | Cleanup stock reservations hết hạn |
...
> Tất cả Admin API yêu cầu JWT Admin Cookie (`nike_admin_session`).
