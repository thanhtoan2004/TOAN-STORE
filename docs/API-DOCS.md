...
Tài liệu API đầy đủ cho TOAN E-commerce. Tất cả API sử dụng prefix `/api/`.
...
| GET | `/api/promo-codes/available` | Danh sách mã giảm giá công khai (Promo Codes) |
| POST | `/api/promo-codes/validate` | Kiểm tra mã (`{ "code": "NIKE2024", "cartTotal": 1000000 }`) |
| GET | `/api/promo-codes/history` | Lịch sử sử dụng mã (Yêu cầu Auth) |
