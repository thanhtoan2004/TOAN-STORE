...
| Method       | Endpoint           | Description                                |
| ------------ | ------------------ | ------------------------------------------ |
| GET/POST/PUT | `/api/admin/menus` | Quản lý Menu đa cấp (Parent/Child + VI/EN) |
| DELETE       | `/api/admin/menus` | Xóa mục menu (`?id=1`)                     |
| GET/POST/PUT | `/api/admin/pages` | Quản lý trang CMS (Policies, Terms)        |
| DELETE       | `/api/admin/pages` | Xóa trang (`?id=1`)                        |
...
## 20. 🏗️ Global Site Data

| Method | Endpoint         | Description                                                                                    |
| ------ | ---------------- | ---------------------------------------------------------------------------------------------- |
| GET    | `/api/site-data` | Tải dữ liệu toàn cục (Menus Tree, Site Settings) cho Header & Footer. Gồm VI/EN Titles + Logo. |
...
## 21. ⚡ Real-time Synchronization (`Socket.io`)

Hệ thống sử dụng **Socket.io** kết hợp **Redis Pub/Sub** để đồng bộ dữ liệu tức thì giữa Server và Client.

| Event          | Direction | Description                                                                |
| -------------- | --------- | -------------------------------------------------------------------------- |
| `notification` | Server →  | Gửi thông báo đẩy tức thì (Voucher mới, trạng thái đơn hàng)               |
| `stock-update` | Server →  | Cập nhật số lượng tồn kho theo thời gian thực tại trang Chi tiết sản phẩm  |
| `join-user`    | Client →  | Client gửi ID người dùng để join vào Room riêng nhận thông báo cá nhân hóa |

**Cơ chế bảo mật**:

- Kết nối Socket chỉ được chấp nhận nếu origin hợp lệ (CORS).
- Các sự kiện nhạy cảm được xác thực qua JWT trước khi phát đi qua Redis Pub/Sub.

---

## 22. 📱 Progressive Web App (`PWA`)

Dự án được cấu hình là một PWA hoàn chỉnh với tính năng Offline-first.

- **Service Worker**: `/api/sw` (Sử dụng Serwist v9).
- **Manifest**: `/manifest.json` định nghĩa Icons, Theme Color (Nike style) và Splash Screen.
- **Caching Strategy**:
  - **Stale-While-Revalidate**: Cho các file tĩnh và API sản phẩm.
  - **Network-First**: Cho các API nhạy cảm (Cart, Auth).
- **Offline Support**: Cho phép người dùng duyệt sản phẩm đã xem khi không có mạng.
