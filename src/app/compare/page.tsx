...
export default function ComparePage() {
    ...
    const hasDiscount = (p: CompareProduct) => (p.sale_price || 0) > 0 && (p.sale_price || 0) < (p.price || 0);
    const discountPercent = (p: CompareProduct) =>
        hasDiscount(p) ? Math.round((((p.price || 0) - (p.sale_price || 0)) / (p.price || 1)) * 100) : 0;
    ...
    const attributes = [
        ...
        },
        {
            ...
            getValue: (p: CompareProduct) =>
                hasDiscount(p) ? `-${discountPercent(p)}%` : '—',
        },
        {
                ...
                {items.length < 2 ? (
                    <div className="text-center py-20 bg-white rounded-2xl">
                        ...
                        <h2 className="text-lg font-bold mb-2">
                            {isVi ? 'Chọn ít nhất 2 sản phẩm để so sánh' : 'Select at least 2 products to compare'}
                        </h2>
