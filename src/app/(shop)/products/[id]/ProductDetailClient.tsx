                            ...
                            <div className="border-t pt-6">
                                ...
                                <div className="grid grid-cols-5 gap-2">
                                    {sizes.map((sizeObj) => (
                                        <button
                                            key={sizeObj.size}
                                            className={`border-2 px-4 py-3 rounded-lg text-center font-medium transition-all ${sizeObj.stock <= 0 && !sizeObj.allow_backorder
                        ...
                        <div className="bg-gray-50 rounded-lg p-6 mb-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="text-center">
                                    ...
                                    <p className="text-gray-600">{reviewStats.total_reviews} {t.product.reviews}</p>
                                </div>
...
function ProductPrintSection({ product, selectedSize }: { product: any; selectedSize: string | null }) {
    return (
        <div className="max-w-4xl mx-auto py-10 px-8 text-black print-catalog-root border">
            ...
            <div className="flex justify-between items-center border-b-2 border-black pb-6 mb-8">
                ...
                <div className="text-right">
                    <p className="text-xs font-bold text-gray-400">Printed: {new Date().toLocaleDateString('vi-VN')}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-12">
                ...
                <div>
                    ...
                    {product.is_new_arrival && (
                        <div className="inline-block border-2 border-black px-4 py-1 text-sm font-bold uppercase mb-4">Mới / New Arrival</div>
                    )}
                </div>
                ...
                <div className="space-y-6">
                    <div>
                        <p className="text-xs uppercase text-gray-400 font-bold mb-1 tracking-widest">{product.category}</p>
                    ...
                    <div className="text-2xl font-bold text-black border-y border-gray-100 py-4">
                        {product.retail_price || product.price ? formatCurrency(product.retail_price || product.price) : 'Liên hệ'}
                    </div>
                    ...
                    <div className="space-y-4 pt-2">
                        <h3 className="font-bold border-b pb-2 text-sm uppercase text-gray-500 tracking-wider">Mô tả sản phẩm</h3>
            ...
            {product.attributes && product.attributes.length > 0 && (
                <div className="mt-12">
                    <h3 className="font-bold border-b-2 border-black pb-3 mb-4 text-lg uppercase tracking-wider">Thông số kỹ thuật / Specifications</h3>
                    <div className="grid grid-cols-2 gap-x-12 gap-y-3">
                        {product.attributes.map((attr: any, idx: number) => (
                            <div key={idx} className="flex justify-between items-center py-2 border-b border-gray-100 italic text-sm">
                                <span className="text-gray-500 font-medium">{attr.name}</span>
                                <span className="font-bold text-gray-800">{attr.option_label || attr.value_text || '—'}</span>
                            </div>
                        ))}
                    </div>
                </div>
