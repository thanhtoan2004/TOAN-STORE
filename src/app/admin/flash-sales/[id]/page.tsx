...
export default function FlashSaleDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const addItemToFlashSale = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setNewItem({ productId: 0, productName: '', flashPrice: 0, quantityLimit: 100, discountPercentage: 0 });
                setSearchTerm('');
                setSearchResults([]);
                fetchFlashSale();
            }
    ...
    if (loading) return <AdminLayout><div className="text-center py-20">Đang tải...</div></AdminLayout>;
    if (!flashSale) return <AdminLayout><div className="text-center py-20">Không tìm thấy Flash Sale</div></AdminLayout>;
                            ...
                            <form onSubmit={handleUpdateFlashSale} className="space-y-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        onChange={(e) => setFlashSale({ ...flashSale, is_active: e.target.checked ? 1 : 0 })}
                                    />
                                ...
                                <div className="mb-8 p-4 bg-gray-50 border rounded-lg space-y-4">
                                        ...
                                        <div className="max-h-40 overflow-y-auto border rounded bg-white divide-y">
                                            {searchResults.map((p) => (
                                                <div
                                                    ...
                                                    onClick={() => setNewItem({ ...newItem, productId: p.id, productName: p.name, flashPrice: p.base_price * 0.5 })}
                                                >
                                                    <div>
                                                        <div className="text-sm font-medium">{p.name}</div>
                                                        <div className="text-xs text-gray-500">{p.sku} - Gốc: {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(p.base_price)}</div>
                                                    </div>
                                                    {newItem.productId === p.id && <div className="text-green-600 text-sm font-bold">Đã chọn</div>}
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {newItem.productId > 0 && (
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t">
                                            <div>
                                                <label className="block text-xs font-medium text-gray-500">Giá Flash Sale (VNĐ)</label>
                                                <input
                                                    type="number"
                                                    className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm"
                                                    value={newItem.flashPrice}
                                                    onChange={(e) => setNewItem({ ...newItem, flashPrice: Number(e.target.value) })}
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-medium text-gray-500">Giới hạn số lượng</label>
                                                <input
                                                    type="number"
                                                    className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm"
                                                    value={newItem.quantityLimit}
                                                    onChange={(e) => setNewItem({ ...newItem, quantityLimit: Number(e.target.value) })}
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-medium text-gray-500">% Giảm giá (Hiển thị)</label>
                                                <input
                                                    type="number"
                                                    className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm"
                                                    value={newItem.discountPercentage}
                                                    onChange={(e) => setNewItem({ ...newItem, discountPercentage: Number(e.target.value) })}
                                                />
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex justify-end gap-2 mt-4">
                                        <button onClick={() => setShowAddForm(false)} className="px-4 py-2 text-sm text-gray-600">Hủy</button>
                                        <button
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Giá Flash</th>
                                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">SL/Đã bán</th>
                                            <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Xóa</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {flashSale.items.map((item) => (
                                            <tr key={item.id}>
                                                ...
                                                <td className="px-4 py-4 text-sm font-bold text-red-600">
                                                    {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.flash_price)}
                                                    <span className="ml-1 text-[10px] text-gray-400 font-normal">(-{item.discount_percentage}%)</span>
                                                </td>
                                        ...
                                        {flashSale.items.length === 0 && (
                                            <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500">Chưa có sản phẩm nào trong đợt này</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
