...
export default function FlashSalesAdminPage() {
    ...
    const getStatus = (fs: FlashSale) => {
        ...
        if (fs.is_active === 0) return { label: 'Vô hiệu hóa', color: 'bg-gray-100 text-gray-800', icon: <XCircle className="w-4 h-4" /> };
        if (now < start) return { label: 'Sắp diễn ra', color: 'bg-blue-100 text-blue-800', icon: <Clock className="w-4 h-4" /> };
        if (now > end) return { label: 'Đã kết thúc', color: 'bg-red-100 text-red-800', icon: <CheckCircle className="w-4 h-4" /> };
        return { label: 'Đang diễn ra', color: 'bg-green-100 text-green-800', icon: <Plus className="w-4 h-4 animate-pulse" /> };
    };
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="px-6 py-4 text-center">Chưa có đợt Flash Sale nào</td>
                                </tr>
                            ) : (
                                flashSales.map((fs) => {
                                    ...
                                    return (
                                        <tr key={fs.id}>
                                            <td className="px-6 py-4">
                                                ...
                                                <div className="text-xs text-gray-500 truncate max-w-xs">{fs.description}</div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right space-x-2">
                                                ...
                                                >
                                                    {fs.is_active === 1 ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
                                                </button>
                                                <Link
