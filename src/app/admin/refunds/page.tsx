...
export default function AdminRefundsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'rejected':
        return (
          <span className="bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs">Từ chối</span>
        );
      default:
        return (
          <span className="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs">{status}</span>
        );
    }
  };
                ...
                refunds.map((refund) => (
                  <tr key={refund.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-gray-900 line-clamp-2" title={refund.reason}>
                        {refund.reason}
                      </p>
