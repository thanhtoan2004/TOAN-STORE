...
import {
    CreditCard, RefreshCw, Star, Gift, ArrowUpRight, ArrowDownLeft,
...
export default function TransactionsPage() {
    ...
    const fetchTransactions = useCallback(async (page: number = 1, type: string = '') => {
        try {
            ...
            if (!res.ok) {
                if (res.status === 401) {
                    setError(isVi ? 'Vui lòng đăng nhập để xem lịch sử giao dịch' : 'Please log in to view transaction history');
                    return;
                }
    ...
    const getAmountDisplay = (tx: Transaction) => {
        if (tx.transaction_type === 'points') {
            ...
            return (
                <span className={`flex items-center gap-1 font-bold ${isEarn ? 'text-green-600' : 'text-red-500'}`}>
        ...
        return (
            <span className={`flex items-center gap-1 font-bold ${isIncoming ? 'text-green-600' : 'text-gray-900'}`}>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 bg-white rounded-xl">
                        ...
                        <p className="text-gray-500 text-sm mb-6">
                            {isVi ? 'Lịch sử giao dịch sẽ hiển thị ở đây' : 'Your transaction history will appear here'}
                        </p>
                        <Link
                            href="/"
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-0.5">
                                                ...
                                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${getStatusBadge(tx.transaction_type, tx.transaction_status)}`}>
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                                ...
                                                {tx.order_number && (
                                                    <>
                                                        <span>•</span>
                                                        <Link href={`/orders/${tx.order_number}`} className="hover:text-black underline">
                                                            #{tx.order_number}
                                                        </Link>
                                                    </>
                                                )}
