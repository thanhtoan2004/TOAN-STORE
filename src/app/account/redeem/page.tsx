...
export default function RedeemPage() {
  ...
  const [selectedVoucher, setSelectedVoucher] = useState<{ id: number; points: number } | null>(
    null
  );
  ...
  const handleRedeemClick = (voucherId: number, pointsRequired: number) => {
    if (currentPoints < pointsRequired) {
      setMessage({ type: 'error', text: 'Bạn không đủ điểm để đổi voucher này!' });
      return;
    }
    setSelectedVoucher({ id: voucherId, points: pointsRequired });
    setModalOpen(true);
  };
  ...
  const confirmRedeem = async () => {
    ...
    setModalOpen(false);
    setRedeeming(selectedVoucher.id);
    setMessage(null);
    ...
    try {
      const response = await fetch('/api/user/redeem', {
        ...
        body: JSON.stringify({ voucherId: selectedVoucher.id }),
      });
      const data = await response.json();
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="mb-8">
          <Link
            ...
            className="text-gray-500 hover:text-black text-sm mb-4 inline-block"
          >
            ← Quay lại Tài khoản
          </Link>
          <h1 className="text-3xl font-bold text-gray-900">Đổi Điểm Thưởng</h1>
          <p className="mt-2 text-gray-600">Sử dụng điểm tích lũy để đổi lấy voucher giảm giá</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-gray-900 to-gray-700 rounded-2xl p-6 mb-8 text-white shadow-lg">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <Link
                href="/account/transactions?type=points"
                className="text-sm text-white/80 hover:text-white underline underline-offset-4 decoration-white/30 hover:decoration-white transition-all"
              >
                Xem lịch sử điểm
              </Link>
            </div>
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      ...
                      <button
                        onClick={() => handleRedeemClick(voucher.id, voucher.points_required)}
        ...
        {/* Info Section */}
        <div className="mt-10 bg-white rounded-xl shadow-sm p-6 mb-8">
        ...
        <AlertModal
          ...
          message={`Bạn có chắc muốn dùng ${selectedVoucher?.points} điểm để lấy voucher này không?`}
          confirmText="Đồng ý, đổi ngay"
          cancelText="Hủy bỏ"
        />
      </div>
    </div>
