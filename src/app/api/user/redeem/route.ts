...
export async function GET() {
    try {
        ...
        const [userData] = await executeQuery(
            'SELECT accumulated_points FROM users WHERE id = ? AND is_banned = 0 AND deleted_at IS NULL',
            [user.userId]
        ) as any[];

        if (!userData) {
            return NextResponse.json({ success: false, message: 'Tài khoản không hợp lệ' }, { status: 403 });
        }

        const currentPoints = userData.accumulated_points || 0;
    ...
    try {
        ...
        try {
            ...
            const [userRows] = await connection.query(
                'SELECT accumulated_points FROM users WHERE id = ? AND is_banned = 0 AND deleted_at IS NULL FOR UPDATE',
                [user.userId]
            ) as any;
            ...
            const currentPoints = userRows[0].accumulated_points || 0;
         ...
         FOR UPDATE`,
            ...
            if (currentPoints < pointsRequired) {
                ...
                connection.release();
                return NextResponse.json(
                    { success: false, message: `Bạn cần ${pointsRequired} điểm nhưng chỉ có ${currentPoints} điểm` },
                    { status: 400 }
                );
            }
            ...
            // Bước 1: Trừ điểm người dùng
            await connection.query(
                'UPDATE users SET accumulated_points = accumulated_points - ? WHERE id = ?',
                [pointsRequired, user.userId]
            );

            // Bước 2: Ghi nhận lịch sử giao dịch điểm
            await connection.query(
                `INSERT INTO point_transactions (user_id, points, type, description, created_at)
         VALUES (?, ?, 'spend', ?, NOW())`,
                [user.userId, -pointsRequired, `Đổi voucher: ${voucher.code}`]
            );
