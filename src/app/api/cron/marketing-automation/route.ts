...
async function handleMonthlyDigests() {
    ...
    const users = await executeQuery<any[]>(`
        SELECT id, email, full_name, membership_tier, accumulated_points
        FROM users
    ...
    for (const user of users) {
        ...
        const pointsNearExpiry = user.accumulated_points > 1000 ? Math.floor(user.accumulated_points * 0.1) : 0;
