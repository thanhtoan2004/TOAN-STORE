...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!storedOtp || storedOtp !== otp.toString()) {
            ...
            return NextResponse.json({ success: false, message: 'Mã OTP không hợp lệ hoặc đã hết hạn' }, { status: 401 });
        }
        ...
        cookieStore.set(AUTH_TOKEN, accessToken, {
            httpOnly: true, path: '/', secure: isProd,
            maxAge: 15 * 60, sameSite: 'strict'
        });
