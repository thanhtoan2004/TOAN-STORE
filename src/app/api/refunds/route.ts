...
import { createRefundRequest, getRefundByOrder, getUserRefunds } from '@/lib/db/repositories/refund';
...
export async function POST(request: Request) {
    try {
        ...
        if (order.status !== 'delivered') {
            return NextResponse.json({ message: 'Chỉ đơn hàng đã giao thành công mới được yêu cầu hoàn tiền.' }, { status: 400 });
        }

        // 2. Check if refund already exists
        const existingRefund = await getRefundByOrder(orderId);
        if (existingRefund) {
            return NextResponse.json({ message: 'Đơn hàng này đã có yêu cầu hoàn tiền đang xử lý.' }, { status: 400 });
        }
