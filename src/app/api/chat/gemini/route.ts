...
const tools = [
  {
    functionDeclarations: [
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            category: {
              type: 'STRING',
              description: 'Danh mục (ví dụ: running, football, basketball, lifestyle, jordan)',
            },
          },
          required: ['category'],
        },
      },
      {
        ...
        description:
          'Tra cứu trạng thái và thông tin chi tiết của một đơn hàng. YÊU CẦU khách hàng cung cấp cả mã đơn hàng và số điện thoại đặt hàng.',
        parameters: {
          type: 'OBJECT',
          properties: {
      ...
      },
      {
        ...
        description:
          'Kiểm tra và chuẩn bị thêm sản phẩm vào giỏ hàng. Dùng khi khách hàng có ý định MUA hàng rõ ràng (ví dụ: "mua đôi này", "lấy size 42").',
        parameters: {
          type: 'OBJECT',
          properties: {
    ...
    if (orderId || phone || isOrderIntent) {
      ...
      for (const msg of recentHist) {
        if (msg.role === 'user') {
          ...
          if (!orderId) {
            const histO =
              contentLower.match(/(nk|nike)\d+_\w+/i) || contentLower.match(/(nk|nike)\d+/i);
      ...
      if (orderId && !phone) {
        ...
        console.log(
          `[Chatbot] Fast Path: Cả 2 thông tin đã đủ. Checking Order ${orderId}, Phone ${phone}`
        );
        try {
        ...
        if (calls.length > 0) {
          const toolPromises = calls.map(async (call: any) => {
            ...
            try {
              ...
              else if (name === 'get_products_by_category')
                data = await getProductsByCategoryForChat(args.category);
              ...
              console.error(`Tool ${name} fail:`, e);
            }
          ...
          const toolResultSend = await chat.sendMessage(
            results.map((r) => r.functionResponse) as any
          );
