...
export async function GET() {
    try {
      ...
      SELECT 
        p.sku, p.name, p.slug, p.base_price, p.retail_price,
 ...
 * 
 * Cấu trúc file:
 * - Cột bắt buộc: name, base_price
 * - Cột tùy chọn: sku, slug, retail_price, short_description, description,
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (rows.length === 0) {
            return NextResponse.json({ success: false, message: 'File không có dữ liệu' }, { status: 400 });
        }

        // Giới hạn số dòng để tránh quá tải Database
        if (rows.length > MAX_ROWS) {
            return NextResponse.json(
                { success: false, message: `Tối đa ${MAX_ROWS} sản phẩm/lần. File có ${rows.length} dòng.` },
                { status: 400 }
            );
        }
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (row.retail_price !== undefined && row.retail_price !== '') {
                ...
                const slug = row.slug?.toString()?.trim() || name.toLowerCase()
                    .normalize('NFD')
                    .replace(/[\u0300-\u036f]/g, '')
                    .replace(/đ/g, 'd')
                    .replace(/[^a-z0-9]+/g, '-')
                ...
                await executeQuery(`
          INSERT INTO products (sku, name, slug, base_price, retail_price, short_description, description, 
