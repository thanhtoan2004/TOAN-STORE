  ...
  try {
    ...
    if (currentCategory) {
      Object.keys(updates).forEach(key => {
        if (updates[key as keyof typeof updates] !== undefined && JSON.stringify(currentCategory[key]) !== JSON.stringify(updates[key as keyof typeof updates])) {
    ...
    if (Object.keys(newValues).length > 0) {
      await logAdminAction(admin.userId, 'UPDATE_CATEGORY', 'categories', id, oldValues, newValues, request);
    }
