...
import { inventory as inventorySchema, productVariants, products, stores as warehouses } from '@/lib/db/schema';
...
export const GET = withPermission('manage:inventory', async (request: NextRequest) => {
  try {
    ...
    const filters = [];
    if (search) {
      filters.push(sql`(${products.name} LIKE ${`%%${search}%%`} OR ${products.sku} LIKE ${`%%${search}%%`} OR ${productVariants.size} LIKE ${`%%${search}%%`})`);
    }
    if (warehouseId) {
...
export const POST = withPermission('manage:inventory', async (request: Request) => {
  try {
    ...
    if (existingInv.length > 0) {
      ...
      await db.update(inventorySchema)
        .set({ quantity: newQty })
      ...
      if (oldQty <= 0 && newQty > 0) {
        ...
        try {
          const product = await db.select().from(products).where(eq(products.id, product_id)).then(r => r[0]);
          if (product) {
            const { wishlists, wishlistItems, users } = await import('@/lib/db/schema');
            const interestedUsers = await db.select({
              email: users.email,
            ...
            if (interestedUsers.length > 0) {
              ...
              interestedUsers.forEach(user => {
                sendWishlistRestockEmail(user.email, user.firstName || 'Customer', product.name, product.id).catch(logger.error);
              });
            }
          }
        } catch (e) {
