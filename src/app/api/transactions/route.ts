  ...
  try {
    ...
    if (!type || type === 'payment') {
      unions.push(`
                SELECT o.id, o.order_number, o.total as amount, o.payment_method,
                       o.status as order_status, o.placed_at as created_at,
    ...
    if (!type || type === 'gift_card') {
      unions.push(`
            ...
            `);
      countUnions.push(
        `SELECT COUNT(*) as cnt FROM gift_card_transactions gct JOIN gift_cards gc ON gct.gift_card_id = gc.id WHERE gc.purchased_by = ?`
      );
      params.push(userId);
      countParams.push(userId);
    }
