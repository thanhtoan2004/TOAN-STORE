...
export const getRedisConnection = () => {
  if (!redisInstance) {
    console.log(
      `[SERVICE_INITIALIZATION] Creating singleton Redis connection to ${REDIS_HOST}:${REDIS_PORT}...`
    );
    ...
    redisInstance.on('error', (err) => {
      ...
      if (
        !(global as any)._lastRedisError ||
        Date.now() - (global as any)._lastRedisError > 10000
      ) {
