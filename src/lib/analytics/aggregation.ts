...
export async function aggregateDailyMetrics(dateString: string) {
  ...
  try {
    ...
    const orderStats = await db
      .select({
        totalRevenue: sql<string>`SUM(CASE WHEN ${orders.status} != 'cancelled' THEN ${orders.totalAmount} ELSE 0 END)`,
        ordersCount: sql<number>`COUNT(*)`,
        cancelledCount: sql<number>`SUM(CASE WHEN ${orders.status} = 'cancelled' THEN 1 ELSE 0 END)`,
      })
      .from(orders)
      .where(and(gte(orders.placedAt, targetDate), lt(orders.placedAt, nextDate)));
    ...
    const customerStats = await db
      ...
      .from(users)
      .where(and(gte(users.createdAt, targetDate), lt(users.createdAt, nextDate)));
