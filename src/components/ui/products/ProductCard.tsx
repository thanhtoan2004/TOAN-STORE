...
interface ProductCardProps {
  ...
  sale_price?: number;
  image_url: string;
  imageUrl?: string; // Add camelCase fallback
  colors?: number;
  is_new_arrival?: boolean;
  isNewArrival?: boolean; // Add camelCase fallback
}
...
const ProductCard = ({
  ...
  // Use final resolved values with fallbacks
  const finalImageUrl = imageUrl || image_url;
  const finalIsNewArrival = isNewArrival || is_new_arrival;
    ...
    <Link href={`/products/${slug || id}`} className="block w-full group">
      <div className="relative cursor-pointer">
        ...
        <div className="relative mb-3 overflow-hidden aspect-square rounded-lg bg-gray-100">
          <Image
            src={
              imageService.getUrl(finalImageUrl, { preset: 'PRODUCT_CARD' }) || '/placeholder.png'
            }
            alt={name}
            fill
            ...
            <div className="text-right">
              {hasDiscount ? (
                <div className="flex flex-col items-start">
                  <span className="font-semibold text-[#e01f3d]">{formatPrice(sale_price)} ₫</span>
