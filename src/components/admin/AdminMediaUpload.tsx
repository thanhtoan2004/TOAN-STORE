...
import { Upload, X, Image as ImageIcon, Video, Loader2, FolderOpen } from 'lucide-react';
...
export default function AdminMediaUpload({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append('folder', 'products'); // Default folder for contextual uploads
  ...
  return (
    <div className={`space-y-2 ${maxWidth}`}>
          ...
          <div className="flex flex-col items-center justify-center text-center">
            {uploading ? (
              ...
              <>
                ...
                <p className="text-xs text-gray-400 mt-1">Images (max 5MB) or Videos (max 50MB)</p>
                <div className="mt-4 border-t border-gray-200 pt-4 w-full">
                  <button
                    type="button"
                    onClick={() => setShowLibrary(true)}
                    className="flex items-center justify-center gap-2 text-blue-600 hover:text-blue-800 text-sm font-semibold w-full"
                  >
                    <FolderOpen size={16} />
                    Chọn từ thư viện
                  </button>
      ...
      <MediaPickerModal
        ...
        onSelect={(m) => {
          setPreviewUrl(m.filePath);
          setMediaType('image'); // Picker only for images mostly right now
          onUploadComplete(m.filePath, 'image');
