  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-white/80 uppercase tracking-[0.2em]">
                  Hạng hiện tại
                </p>
          ...
          <div className="text-left md:text-right bg-black/10 backdrop-blur-sm p-4 rounded-2xl border border-white/10">
            <p className="text-sm font-medium text-white/80 uppercase tracking-widest">
              Điểm khả dụng (Đổi quà)
            </p>
            <p className="text-4xl font-black">{currentAvailablePoints.toLocaleString('vi-VN')}</p>
              ...
              {currentTier?.toLowerCase() !== 'platinum' ? (
                <p className="text-sm font-bold bg-white/20 px-3 py-1 rounded-full backdrop-blur-md">
                  ...
                  <span className="underline decoration-2">
                    {Math.max(0, nextCheckpoint - currentLifetimePoints).toLocaleString('vi-VN')}
                  </span>{' '}
                  điểm để lên hạng kế tiếp
                </p>
              ) : (
                <p className="text-sm font-bold bg-white/20 px-3 py-1 rounded-full backdrop-blur-md">
        ...
        {Object.entries(TIER_CONFIG).map(([key, config], index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 bg-gradient-to-br ${config.gradient} shadow-lg`}
              >
                <Icon />
              </div>

              <h4
                className={`font-black text-lg mb-1 uppercase tracking-tight ${config.textColor}`}
              >
                {config.name.split(' (')[0]}
              </h4>
              ...
              <ul className="space-y-3">
                {config.benefits.map((benefit, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-2 text-xs font-bold text-gray-700 leading-relaxed"
                  >
                    <div
                      className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${isActive ? 'bg-black' : 'bg-gray-300'}`}
                    />
                    {benefit}
                  </li>
                ))}
      ...
      <div className="p-8 bg-black text-white rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
        <div>
          <h3 className="text-xl font-bold mb-2">Đổi điểm nhận quà</h3>
          <p className="text-sm text-gray-400 max-w-md">
            Bạn sẽ nhận được <span className="text-white font-bold">1 điểm cho mỗi 1.000 VNĐ</span>{' '}
            giá trị đơn hàng. Dùng điểm thưởng hiện có để đổi lấy các Voucher giảm giá đặc quyền.
          </p>
        </div>
        <Link
          href="/account/redeem"
          className="px-8 py-4 bg-white text-black rounded-full font-black text-sm uppercase hover:bg-gray-200 transition-colors shrink-0 text-center"
        >
          Đổi Voucher ngay
        </Link>
      </div>
