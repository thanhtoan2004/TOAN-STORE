...
export default function NewsComments({ slug }: { slug: string }) {
  ...
  const CommentItem = ({ comment, isReply = false }: { comment: Comment; isReply?: boolean }) => {
    ...
    return (
      ...
      >
        <div
          className={`flex-shrink-0 ${isReply ? 'w-10 h-10' : 'w-12 h-12'} rounded-full bg-gray-100 overflow-hidden border border-gray-100`}
        >
          {comment.avatar_url ? (
            <img
          ...
          <div className="group relative bg-white border border-gray-100 p-4 rounded-3xl rounded-tl-none shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="font-helvetica-bold text-sm text-gray-900">
                  {comment.user_name}
                </span>
                {isEdited && (
                  <span className="text-[10px] text-gray-400 font-medium italic">(Đã sửa)</span>
                )}
              </div>
            ...
            {editingComment?.id === comment.id ? (
              <form
                onSubmit={handleEdit}
                className="mt-2 bg-gray-50 p-4 rounded-2xl border border-gray-200"
              >
                <textarea
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 mb-10">
        {!isAuthenticated ? (
          <div className="text-center py-6">
            ...
            <p className="text-gray-600 mb-6 font-helvetica-medium">
              Tham gia thảo luận cùng cộng đồng TOAN Store
            </p>
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-between items-center mt-4">
                  <div className="flex items-center text-xs text-gray-400 font-medium">
                    <div
                      className={`w-1 h-1 rounded-full mr-2 ${newComment.length > 900 ? 'bg-red-500' : 'bg-gray-300'}`}
                    />
